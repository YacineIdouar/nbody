...
argsReader (int argc, char **argv)
{
  ...
  docArgs["-dt"] = "select a fixed time step in second (default is "
                   + std::to_string (Dt) + " sec).";
  faculArgs["-ngs"] = "";
  docArgs["-ngs"] = "disable geometry shader for visu (slower but it should "
                    "work with old GPUs).";
  ...
  docArgs["-wg"] = "the size of the OpenCL local workgroup (default is "
                   + std::to_string (LocalWGSize) + ").";
#endif
  faculArgs["s"] = "bodies scheme";
  docArgs["s"]
      = "bodies scheme (initial conditions can be \"galaxy\" or \"random\").";
  faculArgs["-gf"] = "";
  ...
  if (argsReader.parse_arguments (reqArgs, faculArgs))
  ...
  else
    {
      if (argsReader.parse_doc_args (docArgs))
        ...
        std::cout << "A problem was encountered when parsing arguments "
  ...
  if (argsReader.exist_argument ("-soft"))
    {
      ...
      if (Softening == 0.f)
        {
          std::cout << "Softening factor can't be equal to 0... exiting."
                    << std::endl;
          exit (-1);
        }
    }
...
strDate (float timestamp)
{
  ...
  res << std::setprecision (0) << std::fixed << std::setw (4) << days << "d "
      << std::setprecision (0) << std::fixed << std::setw (4) << hours << "h "
      << std::setprecision (0) << std::fixed << std::setw (4) << minutes
      << "m " << std::setprecision (3) << std::fixed << std::setw (5) << rest
      << "s";
...
createImplem ()
{
  ...
  if (ImplTag == "cpu+naive")
    ...
    {
      std::cout << "Implementation '" << ImplTag
...
SpheresVisu *
createVisu (SimulationNBodyInterface *simu)
{
  SpheresVisu *visu;
  ...
  if (VisuEnable)
    {
      ...
      if (GSEnable) // geometry shader = better performances on dedicated GPUs
        visu = new OGLSpheresVisuGS<float> (
            "MUrB n-body (geometry shader)", WinWidth, WinHeight, positionsX,
            positionsY, positionsZ, velocitiesX, velocitiesY, velocitiesZ,
            radiuses, NBodies, VisuColor);
      else
        visu = new OGLSpheresVisuInst<float> (
            "MUrB n-body (instancing)", WinWidth, WinHeight, positionsX,
            positionsY, positionsZ, velocitiesX, velocitiesY, velocitiesZ,
            radiuses, NBodies, VisuColor);
      std::cout << std::endl;
    }
...
main (int argc, char **argv)
{
  ...
  std::cout << "--------------------------------" << std::endl;
  std::cout << "  -> bodies scheme     (-s    ): " << BodiesScheme
            << std::endl;
  ...
  std::cout << "  -> verbose mode      (-v    ): "
            << ((Verbose) ? "enable" : "disable") << std::endl;
  std::cout << "  -> precision                 : "
            << "fp32" << std::endl;
  std::cout << "  -> mem. allocated            : " << Mbytes << " MB"
            << std::endl;
  std::cout << "  -> geometry shader   (--ngs ): "
            << ((GSEnable) ? "enable" : "disable") << std::endl;
  std::cout << "  -> time step         (--dt  ): "
            << std::to_string (Dt) + " sec" << std::endl;
      ...
      if (Verbose)
        {
          ...
          if (ShowGFlops)
            gflops << ", " << std::setprecision (1) << std::fixed
                   << std::setw (6)
                   << perfTotal.getGflops (simu->getFlopsPerIte () * iIte)
                   << " Gflop/s";
          std::cout << "Iteration n°" << std::setw (4) << iIte << " ("
                    << std::setprecision (1) << std::fixed << std::setw (6)
                    << perfTotal.getFPS (iIte) << " FPS" << gflops.str ()
  ...
  if (ShowGFlops)
    gflops << ", " << std::setprecision (1) << std::fixed << std::setw (6)
           ...
           << " Gflop/s";
  std::cout << "Entire simulation took " << perfTotal.getElapsedTime ()
            << " ms "
            << "(" << perfTotal.getFPS (iIte - 1) << " FPS" << gflops.str ()
            << ")" << std::endl;
